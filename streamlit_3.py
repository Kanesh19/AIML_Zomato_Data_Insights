...
if r == 'CRUD':
    ...
    if option == 'CREATE':
        ...
        table_name = st.selectbox("Select Table", ['customer_table', 'restaurant_table', 'order_table', 'delivery_table'])
        ...
        if table_name == 'customer_table':
            ...
            if st.button("Update Customer"):
                ...
                WHERE customer_id = %s
                """
                values = (new_name, new_email, customer_id)
                mycursor.execute(sql, values)
                mydb.commit()
                st.success("Customer Record Updated Successfully")
        
        elif table_name == 'restaurant_table':
            restaurant_id = st.number_input("Enter Restaurant ID", min_value=1, step=1)
            new_name = st.text_input("Enter New Restaurant Name")
        ...
        elif table_name == 'delivery_table':
            delivery_id = st.number_input("Enter Delivery ID", min_value=1, step=1)
            new_status = st.text_input("Enter New Delivery Status")
        ...
        columns = [i[0] for i in mycursor.description]
        df = pd.DataFrame(result, columns=columns)
        st.subheader(f"Updated {table_name} Data")
        st.dataframe(df)
